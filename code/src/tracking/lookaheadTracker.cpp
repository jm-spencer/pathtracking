...
template<> std::array<double,6> LookaheadTracker<6>::getGoalPoint(double robotx, double roboty, double effectiveLookaheadSqr){
  double deltaRX = activeWaypoint[0] - robotx;
  double deltaRY = activeWaypoint[1] - roboty;
  ...
  return {activeWaypoint[0] + lambda * deltaPX,
          ...
          (lambda + 1) * activeWaypoint[4] - lambda * lastWaypoint[4],
          (lambda + 1) * activeWaypoint[5] - lambda * lastWaypoint[5]};
}
